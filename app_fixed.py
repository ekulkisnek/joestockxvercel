...
def render_single_shoe_analysis(result: dict) -> str:
    ...
    <head>
        ...
        <style>
            ...
            .size-title {{ font-weight: bold; color: #333; margin-bottom: 10px; }}
    ...
    top_sizes = performance.get('top_performing_sizes', [])
    if top_sizes:
        html += """
        ...
        for size_data in top_sizes[:5]:
            ...
            html += f"""
                    <tr>
                        ...
                        <td>{'≥' if size_data.get('hit_limit') else ''}{size_data['sales']}</td>
                        <td>{'≥' if size_data.get('hit_limit') else ''}{size_data['velocity']:.2f}</td>
    ...
    if size_breakdown:
        ...
        sorted_sizes = sorted(size_breakdown.keys(), key=lambda x: float(x) if str(x).replace('.', '').isdigit() else 999)
        ...
        for size in sorted_sizes[:12]:  # Show top 12 sizes
            ...
            html += f"""
                <div class="size-card">
                    ...
                    Ask: ${stockx.get('lowest_ask', 'N/A')}<br>
                    <strong>Sales Volume:</strong><br>
                    Count: {volume.get('sales_count', 0)}<br>
                    Velocity: {volume.get('velocity_per_day', 0):.2f}/day<br>
                    <strong>Insight:</strong><br>
                    <em>{insights.get('recommendation', 'No data')}</em>
                </div>
            """
    ...
    alias_pricing = pricing.get('alias_pricing', {})
    if any(alias_pricing.values()):
        html += f"""
            <div class="section">
                <h3>💎 ALIAS PRICING INSIGHTS</h3>
                <table>
                    <tr><th>Metric</th><th>Value</th></tr>
                    <tr><td>Consignment Price</td><td>${alias_pricing.get('consignment_price', 'N/A')}</td></tr>
                    <tr><td>Ship-to-Verify Price</td><td>${alias_pricing.get('ship_to_verify_price', 'N/A')}</td></tr>
                    <tr><td>Lowest Consigned</td><td>${alias_pricing.get('lowest_consigned', 'N/A')}</td></tr>
                    <tr><td>Last Consigned Price</td><td>${alias_pricing.get('last_consigned_price', 'N/A')}</td></tr>
                    <tr><td>Last Consigned Date</td><td>{alias_pricing.get('last_consigned_date', 'N/A')}</td></tr>
                </table>
            </div>
        """
    
    # Data Quality Assessment
    quality_class = 'success' if quality.get('overall_score') == 'Excellent' else ('warning' if quality.get('overall_score') in ['Good', 'Fair'] else 'error')
    html += f"""
            <div class="section {quality_class}">
...
def get_rec_class(recommendation: str) -> str:
    ...
    if 'strong buy' in rec_lower or '🔥' in recommendation:
        ...
        return 'secondary'
    elif 'caution' in rec_lower or 'avoid' in rec_lower or '⚠️' in recommendation:
        return 'warning'
