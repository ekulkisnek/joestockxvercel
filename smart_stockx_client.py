...
# On Vercel, use the stub version that doesn't have http.server
IS_VERCEL = os.getenv('VERCEL') == '1' or os.getenv('VERCEL_ENV') is not None

if IS_VERCEL:
    # Import the Vercel-compatible version (no http.server)
    try:
        from smart_stockx_client_vercel import SmartStockXClient, AuthCallbackHandler, HTTPServer
    except ImportError:
        # Fallback: create minimal stub if import fails
    ...
    # Original implementation with http.server for local/non-Vercel
    import json
    import requests
    import secrets
    import time
